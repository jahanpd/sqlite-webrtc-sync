...
async function build() {
  // 1. Build worker first (needed to generate worker-string.js)
  await esbuild.build({
  ...
  // 2. Copy WASM files and generate worker-string.js BEFORE building main bundle
  copyWASMFiles();
  copySqliteWasmPackage();
  generateWorkerString();

  // 3. Main database bundle (depends on worker-string.ts which was generated above)
  await esbuild.build({
  ...
  // 4. Schema bundle (no React dependency)
  await esbuild.build({
  ...
  // 5. React bundle (React as external peer dependency)
  await esbuild.build({
...
function generateWorkerString() {
  try {
    const workerSrcPath = join(__dirname, 'dist', 'worker.js');
    // Generate to src/ so it can be imported during build
    const workerDestPath = join(__dirname, 'src', 'worker-string.ts');
    
    if (!existsSync(workerSrcPath)) {
      console.warn('worker.js not found, skipping worker-string.ts generation');
      return;
    }
    
    const workerCode = readFileSync(workerSrcPath, 'utf-8');
    
    // Escape the code for use in a template literal
    const escapedCode = workerCode
      .replace(/\\/g, '\\\\')
      .replace(/`/g, '\\`')
      .replace(/\$/g, '\\$');
    
    const output = `// Auto-generated worker code as string - DO NOT EDIT
// This file is generated by build.js from worker/index.ts
export const workerCode = \`${escapedCode}\`;
`;
    
    writeFileSync(workerDestPath, output);
    console.log('Generated worker-string.ts');
  } catch (e) {
    console.warn('Could not generate worker-string.ts:', e.message);
  }
}
