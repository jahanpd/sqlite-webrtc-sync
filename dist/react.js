...
var QueryStore = class {
  ...
  tableToQueries = /* @__PURE__ */ new Map();
  /**
   ...
   */
  subscribe(queryKey, tables, callback) {
  ...
  invalidateTables(tables) {
    const affectedQueries = /* @__PURE__ */ new Set();
    for (const table of tables) {
      const queryKeys = this.tableToQueries.get(table);
      if (queryKeys) {
...
function generateQueryKey(table, options) {
  const parts = [table];
  if (options.where && options.where.length > 0) {
    const wherePart = options.where.map((w) => `${w.column}${w.operator}${JSON.stringify(w.value)}`).join("&");
...
var databaseContexts = /* @__PURE__ */ new Map();
function getOrCreateContext(name) {
...
function DatabaseProvider({
  ...
  }, [name, mode, peerServer, discoveryInterval, schema]);
...
function useDatabaseContext(dbName) {
  if (!dbName) {
    throw new Error(
      'useDatabaseContext requires a database name. Use useDatabaseContext("my-db-name") or access through the database object.'
    );
  }
...
function createQueryBuilder(dbName, tableName, options = { where: [] }) {
  return {
    ...
    exec() {
      return useQueryExec(dbName, tableName, options);
    }
  };
}
...
function useQuery(dbName, tableName) {
  return createQueryBuilder(dbName, tableName);
}
